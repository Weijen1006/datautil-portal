describe("URL Encoder Decoder", () => {
  ...
  it("Encode url correctly", () => {
    cy.get('[data-cy="url-input"]')
      .clear()
      .type(sampleUrl, { parseSpecialCharSequences: false });
    cy.get('[data-cy="url-encode"]').click();
    cy.get('[data-cy="url-encoded-output"] textarea').should(
      "have.value",
      encodedUrl
    );
  });
  ...
  it("Decode url correctly", () => {
    cy.get('[data-cy="url-input"]')
      .clear()
      .type(encodedUrl, { parseSpecialCharSequences: false });
    cy.get('[data-cy="url-decode"]').click();
    cy.get('[data-cy="url-decoded-output"] textarea').should(
      "have.value",
      sampleUrl
    );
  });
  ...
  it("Shows error on decoding url", () => {
    cy.get('[data-cy="url-input"]').type("%ZZ", {
